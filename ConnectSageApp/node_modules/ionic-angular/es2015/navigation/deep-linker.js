...
export class DeepLinker {
    ...
    navChange(direction) {
        if (direction) {
            const /** @type {?} */ rootNavContainers = this._app.getActiveNavContainers();
            ...
            let /** @type {?} */ segments = [];
            for (const /** @type {?} */ rootNavContainer of rootNavContainers) {
                if (isTabs(rootNavContainer) || ((rootNavContainer)).isTransitioning()) {
                    return;
                }
                const /** @type {?} */ segmentsForNav = this.getSegmentsFromNav(rootNavContainer);
